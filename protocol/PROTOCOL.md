...
**2026-08-30**. Local open-weight model replaced: gpt-oss:20b to qwen2.5-coder:14b. The registered local arm model (gpt-oss:20b via Ollama, selected 2026-08-20) produced no parseable action in any pilot trial (4 of 4 terminated malformed_output) and no non-empty final message in any of 13 diagnostic calls under the frozen harness prompt, across default settings, reasoning-effort settings (low, high, disabled), constrained JSON decoding, and a refreshed model pull (Ollama 0.33.2): all generated tokens remained in the model's reasoning channel and generation stopped before a final message was emitted. A simple single-instruction prompt outside the harness format produced normal output, so the failure is specific to operating the action schema. An arm with a zero action rate measures the harness format rather than the registered signal contrasts, and its registered purpose (a reproducible, pinned-weights arm) is not served by 1,200 trials documenting format non-compliance. Replacement selected by the same on-device procedure as the original (probe under the frozen prompt text, unchanged): qwen2.5-coder:14b produced valid actions in 2 of 2 three-turn probe conversations; the 2026-08-20 runner-up devstral:24b remains excluded on its recorded wall-clock and memory pressure. Changed: the local_openweight model_string in the working configuration. Unchanged: prompt text v1.0, conditions, harnesses, trial volumes, all other arms, the deposited bundle (which retains the original string as what was registered). Probe outputs and pilot trial artifacts are preserved with the released materials. Approved by the researcher before the edit.